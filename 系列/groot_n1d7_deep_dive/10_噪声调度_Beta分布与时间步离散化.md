...
- [Beta 分布：专门描述 [0,1] 区间随机变量的分布](/前置知识/002p_前置知识_Beta分布与有界随机变量)（前置知识，建议先读这篇再看本章的 Beta(1.5, 1.0)）
...
如果你不熟悉 [Beta 分布](/前置知识/002p_前置知识_Beta分布与有界随机变量)，先读一下这篇前置知识——简单说，Beta 分布是专门给"只能取 $[0,1]$ 之间的值"这种随机变量设计的分布（不像高斯分布覆盖整条实数轴），$\alpha,\beta$ 两个参数决定密度往哪一侧堆积。

$\text{Beta}(1.5, 1.0)$ 的概率密度：
...
**逐项拆解**（详细推导见 [Beta 分布](/前置知识/002p_前置知识_Beta分布与有界随机变量#2-beta-分布的-pdf) 第 2 节，这里只给结论）：

| 符号 | 含义 | 在本文场景中的值 |
|------|------|-----------------|
| $s$ | 采样得到的随机变量，$\in[0,1]$ | 后面会被变换成时间步 $t$ |
| $\alpha=1.5$ | 控制密度往 $s=1$ 堆积的程度 | $\alpha>\beta$，所以整体偏向大值 |
| $\beta=1.0$ | 控制密度往 $s=0$ 堆积的程度 | $(1-s)^{\beta-1}=(1-s)^0=1$，这一项直接消失，不产生额外偏向 |
| $B(\alpha,\beta)=0.667$ | 归一化常数 | 保证 $\int_0^1 f(s)\,ds=1$ |

**具体数值**：
- $P(s < 0.25) \approx 0.13$（13% 概率采到小值）
- $P(s > 0.75) \approx 0.35$（35% 概率采到大值）
- 均值 = $\alpha/(\alpha+\beta) = 1.5/2.5 = 0.6$（公式来源见 [Beta 分布 第 4 节](/前置知识/002p_前置知识_Beta分布与有界随机变量#4-均值和方差)）
