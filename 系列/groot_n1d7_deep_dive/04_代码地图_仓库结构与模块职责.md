...
数据管线里流转的不是原始的图像/关节角数组，而是统一封装成一个 `VLAStepData` 对象——
这样下游的 Processor 和 Collator 不需要关心具体是哪种机器人、哪种数据格式，只需要按
固定的字段名取数据。每一步的训练数据结构如下：
...
`Gr00tPolicy` 是给下游使用者的推理入口，屏蔽了模型内部的所有细节——调用方只需要提供
观测（图像、状态、语言指令），拿到反归一化后的真实动作即可。典型用法是在机器人的控制
循环里每个周期调用一次：
