...
### 9.3 真实部署代价：谁需要人工介入？谁靠仿真的"免费重置"？

上面的对比表只回答了"用什么算法"，但没回答一个更现实的问题：**这套方法真的能在真实机器人上无人值守地跑起来吗？**

答案是：**本文列出的方法里，大多数训练主战场是 LIBERO / MetaWorld 仿真**。在仿真里，"环境重置"只是调一次 API（`env.reset()`），瞬间完成、零成本、不需要人。这就是为什么这些论文原文（以及我们上面的精读）几乎不讨论"人工介入"问题——**这个问题在仿真里本来就不存在**。它只在少数几篇真正上真机的工作里才会出现，而且往往被埋在正文某一段，没有被提炼成显著的结论。

下表把这个维度单独拎出来：

| 方法 | 训练/验证环境 | Episode 怎么重置 | 是否需要人工在线介入 | 是否依赖特权信息/规划器 |
|------|--------------|-----------------|---------------------|----------------------|
| VLA-RL、TGRPO、RAPT、iRe-VLA、SRPO、BootRL、Residual RL | LIBERO / MetaWorld 仿真 | `env.reset()` 瞬间完成 | 否 | 否（Residual RL 若做仿真→真机蒸馏则涉及特权状态，见下） |
| **SimpleVLA-RL** | LIBERO/MetaWorld 仿真 + Franka 真机 | 仿真自动重置；真机部分论文未明确说明重置方式，只提到"少量在线 fine-tuning" | 未明确说明（**信息缺口**） | 否 |
| **PA-RL** | 真机 | 论文强调"首个真机 RL 微调 7B VLA"，但重置机制未在本文详细展开 | 未明确说明（**信息缺口**） | 否 |
| **ConRFT** | 真机 | 每个 episode 结束需要物理复位物体 | **是**——训练全程人类实时监控，危险动作时按钮接管，接管的动作被记为高奖励数据 | 否 |
| **AGILE（拾放+VLA 阶段）** | 仿真训练 RL Expert，用于生成 VLA 数据 | 仿真自动重置 | 否（数据采集阶段无需人工遥操作） | **是**——RL Expert 训练时用"物体真实位姿、手-物距离"等特权信息，VLA 微调时才换成纯视觉输入 |
| **023 Object-Centric Residual RL** | 仿真训练，零迁移到真机部署 | 仿真自动重置 | 否 | **是**——训练阶段用 ground truth pose，这是它和"真实环境中训练 residual RL"（表中对比的另一行，需要 20 小时且不安全）的关键区别 |
| **IG-RFT** | 真实机器人（4 个长 horizon 任务） | 论文提供了 4 个真实任务 85% 成功率的数据，但重置流程未在本文展开 | 未明确说明（**信息缺口**） | 否 |

**这张表暴露的三件事**：

1. **"仿真训练"和"不需要人工介入"经常被混为一谈**，但因果关系是反的——不是方法设计上解决了人工介入问题，而是仿真环境天然没有这个问题。一旦这些方法真的要上真机，reset 谁来做、要不要人工纠正，都是**未被验证的开放问题**。
2. **特权信息（privileged information）是另一种隐藏的"人工"**——AGILE 和 023 都在训练阶段偷偷用了仿真器才有的上帝视角信息（物体精确位姿），这不算"人工介入"，但意味着这套 RL Expert 换到没有精确位姿追踪系统的真实场景就跑不起来，必须先解决感知问题或做蒸馏。
3. **"未明确说明"本身就是重要信息**：SimpleVLA-RL、PA-RL、IG-RFT 在真机部分给了成功率数字，但都没有交代 episode 之间物体怎么复位。这不代表它们隐瞒了什么——大概率是原论文正文里也没写全，只在附录或代码里体现。但对你的判断很关键：**这些真机结果的"无人化程度"目前没有被充分验证过，不能默认它是全自动的**。

**结论**：如果你要找"真正做到 reset-free + 无人工介入 + 不靠特权信息"的方法，上面这批 VLA-RL 后训练论文里**没有一个满足全部三个条件**。真正专门解决这个问题的是另一条独立的研究线（reset-free RL、SERL 这类），和"VLA + RL 后训练"是两个不完全重叠的方向。
