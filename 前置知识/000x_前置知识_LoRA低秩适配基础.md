...
前向传播是 $h = W_0 x + \frac{\alpha}{r}BAx$，反向传播时需要知道 loss 对 $A$、$B$ 分别怎么变化才能用梯度下降更新它们。对 $h=W_0x+\frac{\alpha}{r}BAx$ 分别关于 $B$、$A$ 用链式法则求导，得到：
...
> **一句话**：反向传播时上游传来的梯度信号 $\partial\mathcal{L}/\partial h$ 只会用来更新 $A$ 和 $B$ 这两个小矩阵，完全不会碰 $W_0$。

**逐项拆解**：

| 符号 | 数学含义 | 具体是什么 |
|------|---------|-----------|
| $\mathcal{L}$ | 训练损失（如交叉熵） | 反向传播的起点 |
| $\partial\mathcal{L}/\partial h$ | loss 对该层输出 $h$ 的梯度 | 由更靠后的层反向传播过来，维度 $d$（与 $h$ 相同） |
| $Ax$ | $A$ 对输入 $x$ 的投影结果 | 维度 $r$（低秩空间中的中间表示） |
| $(Ax)^\top$ | $Ax$ 的转置 | 用于和 $\partial\mathcal{L}/\partial h$ 做外积，得到 $B$ 的梯度（维度 $d\times r$，与 $B$ 形状一致） |
| $B^\top$ | $B$ 的转置 | 把上游梯度从 $d$ 维空间"传回"$r$ 维空间 |
| $x^\top$ | 输入 $x$ 的转置 | 与传回的 $r$ 维梯度做外积，得到 $A$ 的梯度（维度 $r\times k$，与 $A$ 形状一致） |
| $\frac{\alpha}{r}$ | 缩放因子 | 前向传播时乘了这个系数，链式法则求导时原样乘回梯度里 |

**代入数字**：取 $d=4, k=4, r=2, \alpha=4$（缩放因子 $\alpha/r=2$）。假设 $x=[1,0,1,0]^\top$，$A=\begin{bmatrix}0.1&0&0.2&0\\0&0.1&0&0.2\end{bmatrix}$，算出 $Ax=[0.1+0.2,\ 0+0]=[0.3,0]^\top$（2 维）。假设上游传回的梯度 $\partial\mathcal{L}/\partial h = [0.5,-0.2,0.1,0.3]^\top$（4 维）。则：

$$
\frac{\partial\mathcal{L}}{\partial B} = 2\times\frac{\partial\mathcal{L}}{\partial h}\times(Ax)^\top = 2\times\begin{bmatrix}0.5\\-0.2\\0.1\\0.3\end{bmatrix}\times[0.3,\ 0] = \begin{bmatrix}0.3&0\\-0.12&0\\0.06&0\\0.18&0\end{bmatrix}
$$

这是一个 $4\times 2$ 矩阵，形状与 $B$ 完全一致，可以直接用来做梯度下降更新 $B$。$A$ 的梯度同理，用 $B^\top$ 把 4 维梯度先压回 2 维再乘 $x^\top$，最终得到 $2\times 4$ 的矩阵（与 $A$ 形状一致）。

**为什么是这个形式**：由于 $W_0$ 被冻结，它不需要计算梯度，也不需要存储优化器状态（如 Adam 的一阶和二阶动量），这是 LoRA 节省显存的关键原因——反向传播的计算图中 $W_0x$ 这一支路直接被截断，梯度只沿着 $BAx$ 这一支路流动，且流动路径始终保持在 $r$ 维的低秩空间里，计算量远小于对整个 $d\times k$ 的 $W_0$ 求梯度。
...
训练时把 $W_0x$ 和 $\frac{\alpha}{r}BAx$ 分开算是为了让 $W_0$ 保持冻结、节省显存；但训练结束后不再需要区分"哪部分是可训练的"，可以把两条路径提前加到一起，变成一个和原模型形状完全相同的权重矩阵，这样推理时就不用额外算一条 LoRA 支路：

$$
W_{\text{merged}} = W_0 + \frac{\alpha}{r} \cdot BA
$$

> **一句话**：训练时 $W_0$ 和 $BA$ 是分开算再相加的，现在把它们提前加好存成一个矩阵，效果完全一样，但推理时省了一次额外的矩阵乘法路径。

**逐项拆解**：

| 符号 | 含义 | 具体是什么 |
|------|------|-----------|
| $W_0$ | 原始冻结权重 | $d\times k$ 矩阵 |
| $\frac{\alpha}{r}BA$ | 训练好的低秩增量（带缩放） | 同样是 $d\times k$ 矩阵（$B$ 是 $d\times r$，$A$ 是 $r\times k$，乘出来自动是 $d\times k$） |
| $W_{\text{merged}}$ | 合并后的权重 | 与 $W_0$ 形状完全相同的 $d\times k$ 矩阵 |

**代入数字**：延续前面 $d=k=4096, r=8, \alpha=16$ 的例子，缩放因子 $\alpha/r=2$。$BA$ 是把一个 $4096\times8$ 矩阵和一个 $8\times4096$ 矩阵相乘得到的 $4096\times4096$ 矩阵——虽然 $B,A$ 本身参数量很小（共 6.5 万），但相乘后的 $BA$ 是和 $W_0$ 同样大小的稠密矩阵。合并时只需做一次 $W_0 + 2\times BA$ 的矩阵加法（逐元素相加，$4096\times4096\approx1677$ 万次加法），此后推理时前向传播直接用 $h=W_{\text{merged}}x$，不再需要分别计算 $W_0x$ 和 $BAx$ 两条路径再相加——计算量和一个标准的全连接层完全一样。

**为什么这样做是"无损"的**：因为矩阵乘法对加法满足分配律，$(W_0+\frac{\alpha}{r}BA)x = W_0x+\frac{\alpha}{r}BAx$ 在数学上严格相等，合并只是提前把加法算好，不引入任何数值近似（除了浮点运算本身的精度误差），所以推理效果与训练时完全一致。
